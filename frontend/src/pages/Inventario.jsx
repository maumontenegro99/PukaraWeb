...
function Inventario() {
  ...
  const handleViewUbicacion = (ubicacion) => {
      setSelectedUbicacion(ubicacion);
      setShowUbicacionInfo(true);
      setTimeout(() => setUbicacionInfoVisible(true), 10);
  };

  const handleCloseUbicacionInfo = () => {
      setUbicacionInfoVisible(false);
      setTimeout(() => setShowUbicacionInfo(false), 300);
  };
  ...
  const cancelNewCategoria = () => {
      setShowNewCategoriaForm(false);
      setFormData({ ...formData, categoriaId: '' });
  };

  const cancelNewUbicacion = () => {
      setShowNewUbicacionForm(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const payload = {
            ...
            categoria: finalCategoriaId ? { id: finalCategoriaId } : null,
  ...
  const modalContentStyle = { backgroundColor: 'white', padding: '40px', borderRadius: '20px', width: '500px', boxShadow: '0 15px 30px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto', transform: modalVisible ? 'translateY(0) scale(1)' : 'translateY(-20px) scale(0.95)', opacity: modalVisible ? 1 : 0, transition: 'all 0.3s ease-in-out' };
  const locationCardStyle = { backgroundColor: 'white', padding: '30px', borderRadius: '20px', width: '400px', boxShadow: '0 15px 30px rgba(0,0,0,0.2)', textAlign: 'center', transform: ubicacionInfoVisible ? 'translateY(0) scale(1)' : 'translateY(20px) scale(0.95)', opacity: ubicacionInfoVisible ? 1 : 0, transition: 'all 0.3s ease-in-out' };
  ...
  return (
    <div style={containerStyle}>
      <div style={headerCardStyle}>
        <div><h1 style={titleStyle}>Inventario de Grupo</h1><p style={{color: '#888', margin: '5px 0 0 0', fontSize: '1rem'}}>Control logístico y materiales</p></div>
        ...
        <div style={modalOverlayStyle} onClick={(e) => { if(e.target === e.currentTarget) handleCloseModal() }}>
            <div style={modalContentStyle}>
                ...
                <form onSubmit={handleSubmit} style={{display: 'flex', flexDirection: 'column', gap: '20px'}}>
                    ...
                    {/* FILA SUPERIOR: CATEGORÍA + CANTIDAD */}
                    <div style={{display: 'flex', gap: '15px', alignItems: 'flex-start'}}> {/* <-- ALIGN-ITEMS: FLEX-START es el truco */}
                        <div style={{flex: 1}}>
                            <select name="categoriaId" value={formData.categoriaId} onChange={handleInputChange} style={{padding: '12px', width:'100%', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: showNewCategoriaForm ? '#f0f8ff' : 'white'}}>
                                ...
                                <option value="CREAR_NUEVO" style={{fontWeight: 'bold', color: '#00B4D8'}}>+ Nueva Categoría...</option>
                                ...
                                <div style={miniFormContainerStyle}>
                                    <div style={miniFormStyle}>
                                        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px'}}>
                                            <span style={{fontSize:'0.8rem', color:'#00B4D8', fontWeight:'bold'}}>Nueva Categoría:</span>
                                            <button type="button" onClick={cancelNewCategoria} style={{background: 'none', border:'none', cursor:'pointer', fontWeight:'bold', color: '#666', fontSize: '1.1rem'}} title="Cancelar">✕</button>
                                        </div>
                        ...
                        {/* CANTIDAD (No crecerá) */}
                        <input type="number" name="cantidad" value={formData.cantidad} onChange={handleInputChange} placeholder="Cant." style={{padding: '12px', width:'60px', borderRadius: '8px', border: '1px solid #ddd', height: 'fit-content'}} required min="1" />
                    </div>

                    {/* FILA INFERIOR: ESTADO + UBICACIÓN */}
                    <div style={{display: 'flex', gap: '15px', alignItems: 'flex-start'}}> {/* <-- ALIGN-ITEMS: FLEX-START */}
                        <select name="estado" value={formData.estado} onChange={handleInputChange} style={{padding: '12px', flex:1, borderRadius: '8px', border: '1px solid #ddd'}}>
                        ...
                        <div style={{flex: 1}}>
                            <select name="ubicacionId" value={formData.ubicacionId} onChange={handleInputChange} style={{padding: '12px', width:'100%', borderRadius: '8px', border: '1px solid #ddd', backgroundColor: showNewUbicacionForm ? '#f0f8ff' : 'white'}}>
                                ...
                                <option value="CREAR_NUEVO" style={{fontWeight: 'bold', color: '#00B4D8'}}>+ Nueva Ubicación...</option>
                                <option disabled>----------------</option>
                            ...
                            {showNewUbicacionForm && (
                                <div style={miniFormContainerStyle}>
                                    <div style={miniFormStyle}>
                                        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px'}}>
                                            <span style={{fontSize:'0.8rem', color:'#00B4D8', fontWeight:'bold'}}>Nueva Ubicación:</span>
                                            <button type="button" onClick={cancelNewUbicacion} style={{background: 'none', border:'none', cursor:'pointer', fontWeight:'bold', color: '#666', fontSize: '1.1rem'}} title="Cancelar">✕</button>
                                        </div>
